...
## I4 — Write-only subscription/rename echo properties on both plan view models, plus the parameter and payload chain that exists only to feed them `[x2]`
**Where:** src/Bastet/Models/ViewModels/AzureBulkImportViewModels.cs:123,125,210,211,212; src/Bastet/Models/ViewModels/AzureReconcileViewModels.cs:80,81; src/Bastet/Services/Azure/AzureBulkImportPlanner.cs:49-51; src/Bastet/Services/Azure/AzureReconciler.cs:11,20,21; src/Bastet/Services/Azure/IAzureReconciler.cs:11; src/Bastet/Controllers/AzureController.cs:153; src/Bastet/Views/Azure/BulkImport/_BulkScripts.cshtml:6,97,444,445; src/Bastet/Views/Azure/Reconcile/_ReconcileScripts.cshtml:7,118,133
**Breaks:** Five plan properties are stamped on every preview/commit/scan/delete and read by nothing: BulkImportPlanViewModel.SubscriptionId/.SubscriptionName/.RenameMatchedBastetSubnets and AzureReconcilePlanViewModel.SubscriptionId/.SubscriptionName. A chain exists only to feed them: BuildPlan's subscriptionName parameter (delete path passes null); ReconcileScan's subscriptionName form field; both clients' selectedSubscriptionName variables; the DTO's SubscriptionId/SubscriptionName read only at the planner's copy.
**Repro:** Not-runnable (dead code). Closed enumeration at HEAD: every occurrence of the five properties is a declaration or assignment; zero reads anywhere; the DTO's RenameMatchedBastetSubnets has a real reader at planner :146 and is kept. Write-only from birth (73fc76f).
**Fix:** Delete the five plan properties and their stamps; drop the subscriptionName parameter from IAzureReconciler.BuildPlan, AzureReconciler.BuildPlan and AzureController.ReconcileScan; delete BulkImportSelectionDto.SubscriptionId/.SubscriptionName; remove the selectedSubscriptionName variables and the subscriptionId/subscriptionName payload fields from both wizard clients (the bulk client's selectedSubscriptionId stays — it drives BulkGetVNets; the reconcile request's SubscriptionId stays — it drives GetVNetInventory). Update the test selection builders that set the deleted DTO fields.
**Residue of:** none
