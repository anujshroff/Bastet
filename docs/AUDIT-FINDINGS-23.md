...
## L2 — Client-side second implementations of CIDR containment and IP-integer arithmetic `[x1]` — FIXED (wizard kernel); remainder DEFERRED
_Fixed (wizard kernel) in round 23; Details-engine remainder DEFERRED to docs/DEFERRED-FINDINGS.md (L2-remainder). Kernel: AnnotateAvailability stamps BulkAzureSubnetViewModel.ContainingPrefixes via the single containment predicate (PrefixContainsSubnet -> IsWithin -> IpUtilityService.IsSubnetContainedInParent, which SubnetsWithinPrefix now also reuses); the wizard client groups by s.containingPrefixes and ipToInt/prefixContainsCidr are deleted from _BulkScripts.cshtml._
_Swept: grep confirms zero ipToInt/prefixContainsCidr references remain; no new IP arithmetic introduced (only IsSubnetContainedInParent); camelCase containingPrefixes reaches the client. Deferred half is the Details CIDR-modal engine, which has no server counterpart and needs an endpoint/precomputed-field restructure._
_Verified: build 0/0, 881/881; new test AnnotateAvailability_StampsEachSubnetWithOnlyTheVNetPrefixThatContainsIt reddens when the stamp is mutated to all prefixes; reviewer live-drove /Azure/BulkImport against a created two-prefix Azure VNet — each subnet grouped under only its prefix, import committed the child under the right parent._
_Reviewed: independent reviewer PASS — equivalence argued (Azure-aligned inputs make old client and new server containment coincide), mutation reproduced, live wizard drive, deferral correctly scoped._
**Residue of:** none
...
## I10 — Twelve view-model properties mapped on every request but never rendered anywhere `[x1]` — FIXED
_Fixed in round 23. Deleted the 13 never-rendered properties and their object-initializer mapping lines: DeletedSubnetsViewModel.OriginalId/.OriginalParentId (Delete.cs), HostIpViewModel.ModifiedBy (Read.cs + HostIpController), AllHostIpItemViewModel.ModifiedBy, DeletedHostIpViewModel.Id/.OriginalSubnetId/.CreatedBy/.LastModifiedAt/.ModifiedBy, AllDeletedHostIpItemViewModel.Id/.CreatedBy/.LastModifiedAt/.ModifiedBy. Used the verifier-corrected citations: Delete.cs OriginalId/OriginalParentId only — Description (line 265) is rendered and was KEPT._
_Swept: deleting each property (not just its mapping) makes Razor compilation the oracle — the build reported only the mapping-line CS0117 errors and NO view errors, proving none of the 13 is rendered; kept DeletedSubnetsViewModel.ModifiedBy, Description, and AllDeletedHostIpItemViewModel.OriginalSubnetId (all still rendered). No JSON/JS consumer (all five actions return View())._
_Verified: build 0/0, 880/880 (no test referenced them); reviewer live-drove all five pages (Details host-IP list, AllHostIps, per-subnet DeletedHostIps, AllDeletedHostIps, DeletedSubnets) — every column renders from a kept property, no blanks._
_Reviewed: independent reviewer PASS — per-property static confirmation + live drive; refutation attempts (JSON consumer, surviving binding, silent blank, dropped test) all failed._
**Residue of:** none
