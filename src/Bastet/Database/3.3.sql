...
BEGIN
    ...
    VALUES (N'20250330045219_InitialMigration', N'10.0.10');
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20250406031528_AddHostIpAssignment', N'10.0.10');
END;
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260507143218_AddAzureResourceIdToSubnet'
)
BEGIN
    ALTER TABLE [Subnets] ADD [AzureResourceId] nvarchar(500) NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260507143218_AddAzureResourceIdToSubnet', N'10.0.10');
END;
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260726000148_WidenSubnetNameTo100'
)
BEGIN
    DROP INDEX [IX_Subnets_Name] ON [Subnets];
    DECLARE @var nvarchar(max);
    SELECT @var = QUOTENAME([d].[name])
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Subnets]') AND [c].[name] = N'Name');
    IF @var IS NOT NULL EXEC(N'ALTER TABLE [Subnets] DROP CONSTRAINT ' + @var + ';');
    ALTER TABLE [Subnets] ALTER COLUMN [Name] nvarchar(100) NOT NULL;
    CREATE INDEX [IX_Subnets_Name] ON [Subnets] ([Name]);
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[DeletedSubnets]') AND [c].[name] = N'Name');
    IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [DeletedSubnets] DROP CONSTRAINT ' + @var1 + ';');
    ALTER TABLE [DeletedSubnets] ALTER COLUMN [Name] nvarchar(100) NOT NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260726000148_WidenSubnetNameTo100', N'10.0.10');
END;
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260130070800_AddDataProtectionKeys'
)
BEGIN
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260130070800_AddDataProtectionKeys', N'10.0.10');
END;
