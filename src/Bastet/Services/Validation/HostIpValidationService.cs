...
/// </summary>
public class HostIpValidationService : IHostIpValidationService
{
    ...
    public HostIpValidationService(IIpUtilityService ipUtilityService, BastetDbContext context)
    {
    ...
    public ValidationResult ValidateNewHostIp(string ip, int subnetId)
    {
        ...
        // Get subnet
        Subnet? subnet = _context.Subnets
        ...
        if (_context.HostIpAssignments.Any(h => h.IP == ip))
        {
    ...
    public ValidationResult ValidateHostIpUpdate(string originalIp, UpdateHostIpDto dto, byte[] rowVersion)
    {
        ...
        HostIpAssignment? hostIp = _context.HostIpAssignments
        ...
        if (dto.IP != originalIp)
        {
            ...
            if (_context.HostIpAssignments.Any(h => h.IP == dto.IP))
            {
                result.AddError(IP_ALREADY_ASSIGNED, "This IP address is already assigned");
                return result;
            }
            ...
            // Validate IP is within subnet range
            Subnet? subnet = _context.Subnets.Find(hostIp.SubnetId);
            if (subnet != null)
            {
    ...
    public ValidationResult ValidateIpIsWithinSubnet(string ip, string networkAddress, int cidr)
    {
        ...
        if (!_ipUtilityService.IsIpInSubnet(ip, networkAddress, cidr))
        {
                                                              ...
                                                              int originalCidr, int newCidr)
    {
        ...
        if (newCidr > originalCidr)
        {
            ...
            {
                if (!_ipUtilityService.IsIpInSubnet(hostIp.IP, networkAddress, newCidr))
                {
    ...
    private static bool CompareRowVersions(byte[]? current, byte[]? provided)
    {
        if (current == null || provided == null)
        {
            return false;
        }
        ...
        return current.SequenceEqual(provided);
    }
}
